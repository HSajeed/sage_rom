...
*Repository inspection report — updated 2026-09-06 with Phase 1 Experiment 1 (fixed rank-15 DMD), the full DMD fixed-rank sweep into `results_dmdpod/`, the pod_rank_selection stale-label fix, and confirmation that the ground-truth human review is passed.*
...
4. **DMD blows up on the transient-including window (CONFIRMED 2026-09-05)**: with the new default window, linear DMD extrapolation error is **6.03e9** versus 0.26% on the post-transient window. Cause: 39/63 eigenvalues have magnitude >1 (fastest |λ|=1.0747, growth ~2.88/s ≈ 340× per mode over the 81-step test window) — the linear operator fits the observed transient growth as genuinely unstable eigenvalues and forecasts exponential blowup instead of nonlinear saturation. Interpreted as a category-level limitation of linear DMD on transient→saturation flows, not a tuning problem. **Experiment 1 (fixed rank-15 DMD, 2026-09-06) resolves the rank-vs-inherent ambiguity**: at rank 15 the same window gives recon **0.267** / extrap **0.304** with only 7/15 unstable eigenvalues (max growth ~0.07/s vs 2.88/s at r63) — **moderate degradation that stays bounded** (no blowup), against a rank-15 POD reconstruction bar of **0.0158**. So: at high rank the growing high-rank modes dominate and blow up; at low rank it's a mediocre (30%) but *stable* linear model. The honest characterization is **both**: truncation pollution selected spurious fast-growing high-rank modes (§8.4 note), AND the surviving low-rank linear operator still can't represent transient→saturation amplitude physics (30% error at rank 15 vs 1.6% static POD). The full fixed-rank sweep confirms the rank-pollution half with a rank curve (~0.3 through rank 32 → blowup only at 63, §8.11). Nonlinearly-capable variants (HODMD/kernel DMD — `future.md` §4) are the appropriate test, now against a credible moderate-degradation baseline rather than a strawman. NOTE: fixed-rank low-order DMD on the transient window is now TESTED (this entry's former "untested" caveat retired).
5. **Data-efficiency curve fragility**: energy-threshold rank selection is unstable at small n; curve is single-seed, non-monotonic; README says to inspect selected ranks per n and average over multiple seeds before trusting it. On the transient window DMD's extrapolation error *increases* with training size (0.36 → 6.03e9 at n=20→319), so the curve is meaningful only with fixed-rank-per-n or multi-seed statistics.
6. **Neural ROM long-horizon limitation**: training loss supervises short-rollouts (now horizon 30, up from 10); low train loss ≠ accurate full-length rollout. On the transient-including window it improves to **35.3% extrapolation error** (single seed, GPU-trained; 69% runtime was CPU-only before the CUDA `_resolve_device` change). Still not a credible Arm-1 baseline without multi-seed statistics and convergence tracking.
...
10. **`pod_rank_selection.json` stale snapshot-count label — FIXED 2026-09-06**: `run_phase1.py` hardcoded `training_sizes = [20, 50, 100, min(160, len(train.times))]` while the data-efficiency curve (line 119) used `snapshot_counts = [20, 50, 100, n_train]` (319 on the transient window). So the file's largest entry claimed **160** while actually sampling the full training set — the rank-63 value there only agreed with `dmd_eigenvalues.json: rank_used: 63` (n_train=319) by luck. **Fixed**: `training_sizes = [20, 50, 100, len(train.times)]`, consistent with `snapshot_counts`. Committed `results/` files still carry the old (flagged) label; the correction takes effect on the next canonical run. Do not re-use `results/pod_rank_selection.json` values for n=160 as if labeled.
11. **DMD fixed-rank sweep on the transient window — confirms rank pollution; blowup confined to rank 63 (2026-09-06)**: `run_phase1.py` now sweeps DMD over fixed ranks `[1, 2, 4, 8, 15, 32, rank_99]` (no energy selection; `rank_99`=63) → `phase1/results_dmdpod/dmd_error_vs_rank.csv` + `dmd_rank_sweep.json` (per-rank errors + eigenvalue diagnostics). Result: recon/extrap error is **flat ~0.3 through rank 32** (best at rank 32: **0.222/0.243**), exploding only at rank 63 (**9.23e6 / 6.03e9**); unstable-eigenvalue count grows 1→1→3→3→7→12→**39**. So the §8.4 blowup is **rank pollution** (spurious fast-growing high-rank modes), not inherent transient instability — low-rank linear DMD is mediocre-but-stable (30% error where static POD is 1.6%). Nonlinearly-capable DMD variants (HODMD/kernel DMD, `future.md` §4) remain the next real test, now against a solid rank-32 sweet-spot baseline rather than a strawman. Repeatable: `python run_phase1.py --output-dir results_dmdpod` (NeuralROM hard-gated behind `--with-neural`, default off); figures via `phase1/visualization.py` → `phase1/figures/`. Expt-1 (`results/fixed_rank_dmd.json`) kept as a separate artifact, left untouched.
...
The repository is unusually honest about its own limits — nearly every caveat above is self-documented in docstrings and READMEs. Independent verification found its specific factual claims to be accurate in mechanism and substance. **As of 2026-08-27**: all 6 bug_updates fixes applied, ground truth fully reviewed (10/10 entries reviewed:true), regression suite passes (Gate 1: 100/100/100, detect_modification: both tiers, ROM smoke test: Arm 3 = 7 blocks, all gradients flow). Phase 2 deps installed in `dmdsae` conda env. **As of 2026-09-05**: Phase 1's snapshot window changed to include the transient (default `t_min=None`, 400 snapshots) as a deliberate harder test case, and the Phase 1 real-data run on that window is **complete** (`phase1/results/`, report at `phase1/phase1_report.md`): POD 0.105% @ rank-63, **DMD extrapolation blows up to 6.03e9** (39/63 eigenvalues off the unit circle, growth ~2.9/s — linear DMD cannot represent transient growth → saturation), **NeuralROM improves to 35%** extrapolation (horizon 30, 800 epochs, 2× data). NeuralROM training now auto-selects CUDA (`neural_rom_baseline.py:_resolve_device`, verified on the GTX 1650). **As of 2026-09-06**: NeuralROM parked (decision: move forward on DMD+POD only); **Experiment 1 (fixed rank-15 DMD, `run_fixed_rank_dmd.py` → `results/fixed_rank_dmd.json`)** run: recon **0.267** / extrap **0.304**, no blowup — moderate degradation vs the rank-15 POD bar 0.0158, retiring both the "untested" caveat and the strawman-baseline risk; `pod_rank_selection.json` stale-label bug fixed in code (§8.10); human GT review confirmed passed (§8.1). **Full DMD fixed-rank sweep** (`results_dmdpod/`, `run_phase1.py --output-dir results_dmdpod`, NeuralROM hard-gated behind `--with-neural`): error flat ~0.3 through rank 32 (sweet spot rank 32: 0.222/0.243), blowup only at rank 63 (39 unstable eigenvalues) — **rank pollution confirmed, blowup confined to the high-rank regime**; figures rendered via `visualization.py` → `phase1/figures/` (§8.11). The remaining gap to the project's goal is: (1) linear-vs-transient diagnosis done — next real test is nonlinear-capable DMD variants (HODMD/kernel DMD — scoped in `future.md`) against the new rank-32 sweet-spot baseline, (2) set up OpenFOAM locally for real snapshot generation (unmodified/modified framing to be decided: source-change vs constant-change vs both), (3) multi-seed training and comparison of Arms 1/2/3 (multi-seed concern retired for the deterministic POD/DMD pipeline; still applies to NeuralROM Arm 1 if resurrected). Everything upstream of those steps is scaffolded, tested, and internally consistent.
